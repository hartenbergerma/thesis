...
def lin_interpolation(mu, bands_old, range_new):
    """
    interpolate spectrogram values to new bands
    input:  mu, dictionary with spectrogram values
    ...
    mu_new = np.interp(bands_new, bands_old, mu)
    return mu_new, bands_new
...
def get_extinctions(band_range):
    ...
    cyt_c_ox, _ = lin_interpolation(*read_spectrum('cyt_c_ox_500_1000.txt'), band_range)
    extinction_dict['cyt_c_ox'] = cyt_c_ox * 2.3025851 # absorption to extinction
    # cytochrome c reduced
    cyt_c_red, _ = lin_interpolation(*read_spectrum('cyt_c_red_500_1000.txt'), band_range)
    ...
    cyt_b_ox, _ = lin_interpolation(*read_spectrum('cyt_b_ox_402_997.txt'), band_range)
    extinction_dict['cyt_b_ox'] = cyt_b_ox * 2.3025851 # absorption to extinction
    # cytochrome b reduced
    cyt_b_red, _ = lin_interpolation(*read_spectrum('cyt_b_red_402_997.txt'), band_range)
    ...
    cyt_oxi_ox, _ = lin_interpolation(*read_spectrum('cyt_oxi_ox_520_999.txt'), band_range)
    extinction_dict['cyt_oxi_ox'] = cyt_oxi_ox * 2.3025851 # absorption to extinction
    # cytochrome oxibase reduced
    cyt_oxi_red, _ = lin_interpolation(*read_spectrum('cyt_oxi_red_520_999.txt'), band_range)
    ...
    # hemoglobin
    hb_1, _ = lin_interpolation(*read_spectrum('hb_450_630.txt'),[450,630])
    hb_2, _ = lin_interpolation(*read_spectrum('hb_600_800.txt'),[631,650])
    hb_3, _ = lin_interpolation(*read_spectrum('hb_650_1042.txt'),[651,1042])
    hb, _ = lin_interpolation(np.concatenate((hb_1*2.3025851,hb_2*2.3025851,hb_3*10000)), np.arange(450,1043), band_range)
    extinction_dict['hb'] = hb
    # oxyhemoglobin
    hbo2_1, _ = lin_interpolation(*read_spectrum('hbo2_450_606.txt'),[450,606])
    hbo2_2, _ = lin_interpolation(*read_spectrum('hbo2_600_800.txt'),[607,650])
    hbo2_3, _ = lin_interpolation(*read_spectrum('hbo2_650_1042.txt'),[651,1042])
    hbo2, _ = lin_interpolation(np.concatenate((hbo2_1*2.3025851,hbo2_2*2.3025851,hbo2_3*10000)), np.arange(450,1043), band_range)
    extinction_dict['hbo2'] = hbo2

    # water
    water = lin_interpolation(*read_spectrum('water_600_1050.txt'), band_range)
    extinction_dict['water'] = water

    # fat
    fat = lin_interpolation(*read_spectrum('fat_429_1098.txt'), band_range)
